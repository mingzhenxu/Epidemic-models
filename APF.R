...
tau = 4
M = 26 #times (biweekly reporting so it is 130 biweeks)
J = 150#particles
X = array(0,dim = c((M+1),J,3*5))
# X.true = array(0,dim = c((M+1),3*5))
# proposed_y = array(0,dim = c(M+1,3))
...
# predictive measurement density

predmeas = function(m,j) {
  dt = 1
  Int1 = function(z,k) dnbinom(z,dt/sigma2,1/(sigma2*v_ir*X[m,j,3+(k-1)*5])) *  dmeas(proposed_y[m,k],rho,phi,z+X[m,j,4+(k-1)*5])
...
#### transmission model ####
sir.step <- function(t,R0,a,alpha,mu,v_ei,v_ir,sigma2,rho,phi,G,X){
  for (k in 1:3) {
    vkl = numeric(3)
    ss = numeric(3)
...
# for (k in 1:3) {
#   X.true[1,1,1+(k-1)*5] = round(s*N[round(year[T]-year_0+0.51),(k+1)]) #S
#   X.true[1,1,2+(k-1)*5] = round(e*N[round(year[T]-year_0+0.51),(k+1)]) #E
#   X.true[1,1,3+(k-1)*5] = round(i*N[round(year[T]-year_0+0.51),(k+1)]) #I
#   X.true[1,1,4+(k-1)*5] = 0   #delta N_ir(weekly)
#   X.true[1,1,5+(k-1)*5] = N[round(year[T]-year_0+0.51),(k+1)] #P
# }
# 
# for (m in 1:M) {
#   X.true[m+1,1,] = sir.step(T + m -1,R0,a,alpha,mu,v_ei,v_ir,sigma2,rho,phi,G,X.true) # modify the sir.step function when we generate the true trajecory
...
####initial X####
for (k in 1:3) {
  X[1,,1+(k-1)*5] = rep(round(s*N[round(year[T]-year_0+0.51),(k+1)]),J) #S
  X[1,,2+(k-1)*5] = rep(round(e*N[round(year[T]-year_0+0.51),(k+1)]),J) #E
  X[1,,3+(k-1)*5] = rep(round(i*N[round(year[T]-year_0+0.51),(k+1)]),J) #I
  X[1,,4+(k-1)*5] = rep(0,J)   #delta N_ir(weekly)
  X[1,,5+(k-1)*5] = rep(N[round(year[T]-year_0+0.51),(k+1)],J) #P 
}
...
for(m in 1:M){
  if (m==1){
    ...
    y_n = sapply(1:J, function(j) predmeas(m,j))
  }
  t = T+m-1 #biweekly
  for (j in 1:J) {
    #Proposed X using proposed parameters
    X[m+1,j,] = sir.step(t,R0,a,alpha,mu,v_ei,v_ir,sigma2,rho,phi,G,X)
    w.o[m,j] = prod(sapply(1:3,function(k) dmeas(proposed_y[m,k],rho,phi, X[m+1,j,4+(k-1)*5])))
    y_future[j] = predmeas(m+1,j)
    w_o_apf[m,j] = ifelse(y_future[j]==0,0,ifelse(m==1,w.o[m,j]*y_future[j]/y_n[j], w.o[m,j]*y_future[j]/y_n[j]))
  }
  #calculate true weight
  for (j in 1:J) {
    w[m,j] = ifelse(w_o_apf[m,j]==0,0,w_o_apf[m,j]/sum(w_o_apf[m,]))
    #Filtering failure
    if(w[m,j] > tolerance){
      w[m,j] = w[m,j]
    }else{
...
####London####
par(mfrow=c(2,2))
plot(rowMeans(X[,,6]),xlim=c(0,30),ylim=c(0,210000),type = 'l',main = 'London',xlab = 'Times(Biweek)',ylab='Susceptible Population')
...
legend("topleft", bty="n",legend=c("Est. mean","Median",'10%,90% quantile',"True S"),col=c(1,2,3,4), lty=c(1,1,2,1), cex=0.7)

plot(rowMeans(X[,,7]),xlim=c(0,30),ylim=c(0,40000),type = 'l',main = 'London',xlab = 'Times(Biweek)',ylab='Exposed Population')
...
legend("topleft", bty="n",legend=c("Est. mean","Median",'10%,90% quantile',"True E"),col=c(1,2,3,4), lty=c(1,1,2,1), cex=0.7)

plot(rowMeans(X[,,8]),xlim=c(0,30),ylim=c(0,40000),type = 'l',main = 'London',xlab = 'Times(Biweek)',ylab='Infectious Population')
...
legend("topleft", bty="n",legend=c("Est. mean","Median",'10%,90% quantile',"True I"),col=c(1,2,3,4), lty=c(1,1,2,1), cex=0.7)

plot(rowMeans(X[,,9]),xlim=c(0,30),ylim = c(0,25000),type = 'l',main = 'London',xlab = 'Times(Biweek)',ylab='BiWeekly Recovered')
...
legend("topleft", bty="n",legend=c("Est. mean","Median",'10%,90% quantile',"True N_ir"),col=c(1,2,3,4), lty=c(1,1,2,1), cex=0.7)
...
####Liverpool####
par(mfrow=c(2,2))
plot(rowMeans(X[,,1]),xlim=c(0,30),type = 'l',main = 'Liverpool',xlab = 'Times(Biweek)',ylab='Susceptible Population')
...
plot(rowMeans(X[,,2]),xlim=c(0,30),ylim=c(0,12000),type = 'l',main = 'Liverpool',xlab = 'Times(Biweek)',ylab='Exposed Population')
...
plot(rowMeans(X[,,3]),xlim=c(0,30),ylim=c(0,10000),type = 'l',main = 'Liverpool',xlab = 'Times(Biweek)',ylab='Infectious Population')
...
plot(rowMeans(X[,,4]),xlim=c(0,30),ylim=c(0,8000),type = 'l',main = 'Liverpool',xlab = 'Times(Biweek)',ylab='BiWeekly Recovered')
...
####Manchester####
par(mfrow=c(2,2))
plot(rowMeans(X[,,11]),xlim=c(0,30),type = 'l',main = 'Manchester',xlab = 'Times(Biweek)',ylab='Susceptible Population')
...
plot(rowMeans(X[,,12]),xlim=c(0,30),ylim=c(0,8000),type = 'l',main = 'Manchester',xlab = 'Times(Biweek)',ylab='Exposed Population')
...
plot(rowMeans(X[,,13]),xlim=c(0,30),ylim=c(0,7000),type = 'l',main = 'Manchester',xlab = 'Times(Biweek)',ylab='Infectious Population')
...
plot(rowMeans(X[,,14]),xlim=c(0,30),ylim=c(0,5000),type = 'l',main = 'Manchester',xlab = 'Times(Biweek)',ylab='BiWeekly Recovered')
...
par(mfrow=c(1,1))
plot(proposed_y[,1],xlim=c(0,30),ylim=c(0,11000),type = 'l',main = 'Assigned Data',xlab = 'Times(Biweek)',ylab='Biweekly reported cases',col=1)
lines(proposed_y[,2],col=2)
lines(proposed_y[,3],col=3)
legend("topleft", legend=c("Liverpool",'London','Manchester'),col=c(1,2,3), lty=c(1,1,1), cex=0.7)
